/**
 * AchadosPerdidosPage — Portal do Aluno: Achados e Perdidos
 *
 * Fase 15 — lista objetos disponíveis, permite reivindicar com confirmação de
 * senha, e exibe objetos já reivindicados pelo aluno.
 */
...
function toStudentEmail(enrollmentNumber: string): string {
...
export default function AchadosPerdidosPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    const [settingRes, availRes, myRes] = await Promise.all([
      ...
      supabase
        .from('lost_found_items')
        .select('*')
        .eq('claimed_by_id', student.id)
        .eq('claimed_by_type', 'student')
  ...
  async function handleConfirmClaim() {
    if (!student || !claimItem) return;
    if (!password.trim()) { setPasswordError('Informe sua senha.'); return; }
    setPasswordError('');
    setConfirming(true);

    const email = toStudentEmail(student.enrollment_number);
    ...
    const { error: updateError } = await supabase
      ...
      .update({
        status:          'claimed',
        claimed_by_type: 'student',
        claimed_by_id:   student.id,
        claimed_at:      now,
        claimed_portal:  'student',
      })
      .eq('id', claimItem.id)
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          {student?.full_name && (
            <p className="text-sm text-gray-500 dark:text-gray-400">{student.full_name}</p>
          )}
        </div>
      </div>
