...
const NAV = [
  ...
  { path: '/professor/turmas', label: 'Minhas Turmas',   icon: BookOpen },
  { path: '/professor/planos', label: 'Planos de Aula',  icon: FileText },
  { path: '/professor/provas', label: 'Provas',          icon: FileQuestion },
];

function ProfessorNav({ onClose }: { onClose?: () => void }) {
  const { professor, signOut } = useProfessor();
  ...
  const initials = professor?.full_name
    ? professor.full_name.split(' ').slice(0, 2).map((n) => n[0]).join('').toUpperCase()
    : 'PR';
        ...
        <div className="px-5 py-3 border-b border-white/10">
          <div className="flex items-center gap-2.5">
            {professor.avatar_url ? (
              ...
              <div className="w-8 h-8 rounded-full bg-brand-secondary/20 flex items-center justify-center text-brand-secondary font-bold text-sm">
                {initials}
              </div>
            )}
            <div className="min-w-0">
              <p className="text-white text-xs font-medium truncate">{professor.full_name}</p>
              <p className="text-white/50 text-xs">Professor(a)</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex">
        ...
        {mobileOpen && (
          <div className="md:hidden fixed inset-0 z-40" onClick={() => setMobileOpen(false)}>
            ...
            <aside
              className="absolute left-0 top-0 bottom-0 w-64 bg-brand-primary flex flex-col"
            ...
            >
              ...
              <ProfessorNav onClose={() => setMobileOpen(false)} />
            </aside>
          </div>
        )}
        ...
        <div className="flex-1 md:ml-60 flex flex-col min-h-screen">
          ...
          <header className="md:hidden flex items-center justify-between px-4 py-3 bg-brand-primary text-white sticky top-0 z-20">
            <div className="flex items-center gap-2.5">
              ...
              <span className="font-semibold text-sm">Portal do Professor</span>
            </div>
