...
| OP-1 | Central de Migracao de Dados (Onboarding) | ⚙️ Em andamento (2026-04-17) — PR1: infraestrutura (migration 150, hub `/admin/migracao`, lock por modulo). PR2a: refator para `ModuleImportWizard` generico + lock pos-sucesso (`components/ModuleImportWizard.tsx`, `lib/import-wizard.ts`). PR2b: importadores de Contatos, Fornecedores e Produtos (configs em `lib/import-configs/*`). PR2c: hierarquia academica em 3 importadores separados (Segmentos → Series → Turmas) com locks independentes (migration 151, rotas `/admin/migracao/{segmentos,series,turmas}`). PR3: Grupo B financeiro — Contas a Receber e Contas a Pagar com lookup de categoria contabil por nome (type='receita'|'despesa'), parse BR de data/dinheiro, normalizacao de status/tipo; rotas `/admin/migracao/{receber,pagar}`. PR4: Grupo C — Lancamentos de Caixa (`financial_cash_movements`) com lookup de caixa por nome e calculo de `balance_after` via running sum por caixa partindo de `current_balance`; migration 152 remove `appointments` do escopo (nao vital em ERPs escolares; fica fora da v1 do OP-1). Rota `/admin/migracao/caixa`. PR5: Colaboradores (`profiles` + `auth.users`) — extensao do `ModuleImportWizard` com step opcional "Revisar" (via `perRowOverrides`) onde o admin define o papel (`role`) de cada usuario antes do insert, com seletor "aplicar a todos". Edge Function `bulk-import-users` cria auth rows com senha temporaria + `must_change_password=true`. Rota `/admin/migracao/colaboradores`. **Sprint 10 completo** — 10 importadores ativos, zero modulos pendentes. | Alta | Todas as tabelas-alvo existentes |
