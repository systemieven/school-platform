...
| **PWA / Mobile-First** | Sprint 13 em andamento. PR1 (2026-04-17): `vite-plugin-pwa@1.2.0` instalado, `vite.config.ts` com `VitePWA({ registerType: 'autoUpdate' })`, manifest base (name/short_name/theme=#003366/bg=#f8f7f4/display=standalone/lang=pt-BR), icones SVG (`pwa-icon.svg` any + `pwa-icon-maskable.svg` maskable), workbox precache (89 entries / ~5 MiB) com `navigateFallback: /index.html` e denylist para `/api/` e `/functions/`, `devOptions.enabled=false` (SW so ativa em build). Proximos PRs: manifest dinamico lendo `system_settings`, meta tags iOS + hook de install prompt, revisao mobile-first da `PortariaPage`. | Media | 🚧 Em andamento (Sprint 13 PR1 / 2026-04-17) |
