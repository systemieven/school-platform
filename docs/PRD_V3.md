...
| `ai-billing-sync` | X-Trigger-Secret (cron) OU JWT admin | 17/04 | Sprint 13.IA-dash PR2 | Consulta APIs admin oficiais (Anthropic `/v1/organizations/usage_report/messages`+`/cost_report`, OpenAI `/v1/organization/usage/completions`+`/costs`) para o dia solicitado; UPSERT em `ai_usage_snapshots` por `(provider, snapshot_date)`; inferência heurística de recarga quando `total_spent` do dia anterior > hoje (registra em `ai_recharges` com `source='inferred'`). Body: `{ provider?, date?, force? }`. Pula provider com `status='skipped'` quando admin key ausente. Disparado diariamente às 00:01 UTC via `pg_cron`. |
| `ai-billing-manual-refresh` | JWT admin/super_admin | 17/04 | Sprint 13.IA-dash PR2 | Thin wrapper que delega para `ai-billing-sync` com `force=true`, passando `x-trigger-secret` internamente. Rate-limit de **1 chamada / 5 minutos** baseada no `fetched_at` mais recente em `ai_usage_snapshots` (retorna 429 com `retry_after_seconds`). Permite o botão "Atualizar agora" no futuro `AiUsageDashboard`. |
...
| Sprint 13.IA-dash | Dashboard de Uso/Consumo IA (configuracoes/ia como root) | ⚙️ **PR1 concluido (2026-04-17)** — migrations 165-169 aplicadas em produção: `ai_usage_snapshots` (UNIQUE provider+snapshot_date, RLS admin SELECT), `ai_recharges` (source manual/auto/inferred, RLS admin ALL), `company_ai_config` extendida com `anthropic_admin_api_key`/`openai_admin_api_key`/`openai_organization_id`/`balance_alert_threshold`, RPC `ai_usage_stats(from,to,provider?,agent_slug?)` SECURITY DEFINER retornando `{kpis,daily,top}` com custo estimado por modelo (tabela de preços embutida), job `pg_cron` `ai-billing-sync-daily` agendado para 00:01 UTC via `net.http_post`. `AiAgentsPanel` ganha 4 campos novos no card "Chaves de API" (Anthropic Admin Key, OpenAI Admin Key, Organization ID, Alerta de saldo baixo USD) com toggle Eye/EyeOff e divisor explicativo separando inference keys das admin keys. **PR2 concluido (2026-04-17)**: Edge Function `ai-billing-sync` (deploy v1, `verify_jwt=false` — auth dual via `x-trigger-secret` para cron ou JWT admin para chamada manual; faz `fetch` paralelo de usage+cost por provider, agrega tokens_input/output/requests_count/total_spent_usd, UPSERT em `ai_usage_snapshots`, heurística de recarga baseada em queda de `total_spent_usd` vs. dia anterior registra `ai_recharges.source='inferred'`, pula providers sem admin key com `status='skipped'`). Edge Function `ai-billing-manual-refresh` (deploy v1, `verify_jwt=true` — valida `admin/super_admin`, rate-limit por `fetched_at` do último snapshot com janela de 300s respondendo 429 com `retry_after_seconds`, delega para `ai-billing-sync` via fetch interno com `x-trigger-secret`). PR3 (refactor em sub-abas + AiUsageDashboard) pendente. Detalhes em §10.8B.
